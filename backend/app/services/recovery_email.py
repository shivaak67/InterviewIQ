...
def send_reset_email(recipient: str, token: str) -> None:
    ...
    try:
        if settings.resend_api_key:
            ...
            request = Request('https://api.resend.com/emails', data=json.dumps(payload).encode(),
                headers={'Authorization': 'Bearer ' + settings.resend_api_key,
                    'Content-Type': 'application/json', 'User-Agent': 'PrepPilot/1.0'}, method='POST')
        ...
        logger.error('Password recovery email delivery failed (HTTP %s); check provider configuration.', error.code)
    except Exception:
